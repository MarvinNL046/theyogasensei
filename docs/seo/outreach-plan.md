...
## What you actually do with the list (the workflow)

The CSV (`outreach-targets.csv`) is a **weekly working queue / mini-CRM**. It is not a
to-read list — every row is something you send or do, then log. The loop:

1. **Pick 5–10 rows** per week (start with Priority 1). ~30–45 min/week is enough.
2. **Personalize.** Open the target, add 1–2 specific sentences that prove you read it
   (reference their actual article/page). Generic templates get ignored — the personal
   line is the whole game.
3. **Send** from `hello@theyogasensei.com` (or `info@`). Keep it short.
4. **Log it:** set `Status = sent`, fill `Sent` date, set `Follow-up` to +6 days.
5. **Follow up once** after ~6 days if no reply (one nudge, then stop — never a third).
6. **Record the result:** `won` (+ the live URL), `no` or `ignored`.
...
**Realistic expectation:** this is a slow build. Landing 5–10 genuine links over the next
8–12 weeks is a good result for a 4-week-old site and is enough to start moving DR and
...
your honest angle. When you get quoted, you get a link on a real publication. Check 1–2
...
- **Free Yoga for Beginners Starter Guide** → "yoga for beginners" resource pages.
- **Honest, no-fake-testing reviews** → "best yoga blogs" roundups, gear resource lists.

**Tier 4 — Relationships (slow, high value).** Brands you review, podcasts, teacher
trainings. Warm, not transactional.
...
> theyogasensei.com. Quote me as: Marvin Smit, The Yoga Sensei (https://www.theyogasensei.com).
>
...
> Hi [name],
>
> [Specific line: "I found your page on chair yoga for seniors at [URL] while researching…"]
> — really useful list.
>
> I made a free, printable chair-yoga routine for seniors (large text, no sign-up, no
> catch): [URL]. If it fits, it might be a helpful addition for your readers.
>
...
> Hi [name],
>
> [Specific line referencing a post of theirs.] I write honest, research-based yoga content
...
> Hi [name],
>
> Thanks for mentioning The Yoga Sensei in [post + URL]. Would you mind linking the mention
> to https://www.theyogasensei.com so readers can find it? Totally understand if not.
>
> Marvin
