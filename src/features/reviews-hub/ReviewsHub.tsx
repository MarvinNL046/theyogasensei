      ...
      <section className="border-b border-[color:var(--color-border)] bg-[color:var(--color-surface-muted)]">
        <Container size="wide" className="py-16 md:py-24">
          ...
          <h1 className="mt-5 max-w-4xl font-serif text-4xl leading-[1.05] tracking-[-0.045em] md:text-6xl">
            Clear verdicts, with the limits shown.
          </h1>
          <p className="mt-6 max-w-2xl text-lg leading-relaxed text-[color:var(--color-ink-soft)]">
            Every review states whether I used the mat myself or researched it
            from current documentation and independent evidence. Either way, you
            see who it suits, who should skip it and which compromise matters
            most.
          </p>
        </Container>
      </section>
          ...
          {reviews.length ? (
                    ...
                    <div className="flex flex-1 flex-col p-6">
                      ...
                      <dl className="mt-5 space-y-3 border-t border-[color:var(--color-border)] pt-5 text-sm">
                        <div>
                          ...
                          <dd className="mt-1 text-[color:var(--color-ink-muted)]">
                            {review.bestFor}
                          </dd>
                        </div>
                        <div>
                      ...
                      <div className="mt-auto flex items-center justify-between gap-3 pt-6">
                        <span className="rounded-full bg-[color:var(--color-surface-muted)] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.12em] text-[color:var(--color-ink-soft)]">
                          {review.researchStatus}
                        </span>
                        <span className="inline-flex items-center gap-2 text-xs font-semibold text-[color:var(--color-olive-deep)]">
                          Read review{' '}
            ...
            <div className="rounded-2xl border border-[color:var(--color-border)] bg-white px-6 py-14 text-center">
              <h2 className="font-serif text-2xl">
                No review matches every filter.
              </h2>
              <p className="mt-3 text-sm text-[color:var(--color-ink-muted)]">
                Remove one filter or reset the full view.
              </p>
            </div>
          )}
          <div className="mt-16 rounded-2xl bg-[color:var(--color-olive-deep)] p-7 text-white md:flex md:items-center md:justify-between md:gap-8 md:p-9">
            <div>
              ...
              <h2 className="mt-2 font-serif text-3xl">
                Compare the leading mats side by side.
              </h2>
            </div>
            <Link
              to="/reviews/best-yoga-mats"
              className="mt-5 inline-flex shrink-0 rounded-full bg-white px-5 py-3 text-sm font-semibold text-[color:var(--color-olive-deep)] md:mt-0"
            >
              See the best yoga mats
            </Link>
          </div>
        </Container>
      </section>
