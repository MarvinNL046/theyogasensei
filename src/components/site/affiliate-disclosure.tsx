...
export function AffiliateDisclosure({ compact, className }: AffiliateDisclosureProps) {
  if (compact) {
    return (
      <p
        className={cn(
          'not-prose my-4 rounded-md border-l-2 border-accent-300 bg-accent-50/60 px-4 py-2 text-xs text-stone-700',
          className,
        )}
      >
  ...
  return (
    <aside
      className={cn(
        'not-prose my-6 rounded-lg border-l-4 border-accent-500 bg-accent-50/60 px-5 py-4 text-sm text-stone-800',
        className,
      )}
      role="note"
    >
      <p className="mb-1 font-semibold text-accent-800">Affiliate disclosure</p>
      <p>
        ...
        <Link to="/affiliate-disclosure" className="underline hover:text-accent-800">
