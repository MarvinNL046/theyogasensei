...
export const Route = createFileRoute('/reviews/')({
  validateSearch: (search: Record<string, unknown>): ReviewsSearch => {
    ...
    const useCase =
      typeof search.useCase === 'string' ? search.useCase : undefined
    const price = typeof search.price === 'string' ? search.price : undefined
    const research =
      typeof search.research === 'string' ? search.research : undefined
    return {
      ...(valueFromSlug(REVIEW_BRANDS, brand, 'All brands') === 'All brands'
        ? {}
        : { brand }),
      ...(valueFromSlug(REVIEW_MATERIALS, material, 'All materials') ===
      'All materials'
        ? {}
        : { material }),
      ...(valueFromSlug(REVIEW_USE_CASES, useCase, 'All use cases') ===
      'All use cases'
        ? {}
        : { useCase }),
      ...(valueFromSlug(REVIEW_PRICE_BANDS, price, 'All price bands') ===
      'All price bands'
        ? {}
        : { price }),
      ...(valueFromSlug(
  ...
  head: () =>
    buildHubHead({
      ...
      description:
        'Filter transparent yoga mat reviews by brand, material, use case, price and research status, with clear trade-offs and alternatives.',
      path: '/reviews',
...
function ReviewsIndex() {
  ...
  const filters: ReviewFiltersState = hydrated
    ? {
        ...
        material: valueFromSlug(
          REVIEW_MATERIALS,
          search.material,
          'All materials',
        ),
        useCase: valueFromSlug(
          REVIEW_USE_CASES,
          search.useCase,
          'All use cases',
        ),
        priceBand: valueFromSlug(
          REVIEW_PRICE_BANDS,
          search.price,
          'All price bands',
        ),
  ...
  return (
    <ReviewsHub
      ...
      onChange={(next) =>
        navigate({
          search: {
            ...(next.brand === 'All brands'
              ? {}
              : { brand: filterValueSlug(next.brand) }),
            ...(next.material === 'All materials'
              ? {}
              : { material: filterValueSlug(next.material) }),
            ...(next.useCase === 'All use cases'
              ? {}
              : { useCase: filterValueSlug(next.useCase) }),
            ...(next.priceBand === 'All price bands'
              ? {}
              : { price: filterValueSlug(next.priceBand) }),
